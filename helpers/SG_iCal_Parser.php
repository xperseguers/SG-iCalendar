...
class SG_iCal_Parser
{
    ...
    protected static function UnfoldLines($content)
    {
        ...
        for ($i = 0; $i < count($content); $i++) {
            ...
            while (isset($content[$i + 1]) && strlen($content[$i + 1]) > 0 && (str_starts_with($content[$i + 1], ' ')   || str_starts_with($content[$i + 1], "\t"))) {
    ...
    {
        $main_sections = ['vevent', 'vjournal', 'vtodo', 'vtimezone', 'vcalendar'];
        $array_idents = ['exdate', 'rdate'];
        $sections = [];
        $section = '';
        $current_data = [];
